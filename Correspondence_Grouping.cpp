...
void GeometricConsistency(pcl::PointCloud<PointT>::Ptr& obj_keypoints, pcl::PointCloud<PointT>::Ptr& data_keypoints,
    ...
    std::vector<pcl::Correspondences>& clustered_corrs, float GC_size, int GC_threshold)
{
    pcl::GeometricConsistencyGrouping<PointT, PointT> gc_clusterer;
    gc_clusterer.setGCSize(GC_size);        //consensus set resolution, default = resolution, 越大越松
    gc_clusterer.setGCThreshold(GC_threshold); //minimum cluster size，越小越松 （整数）
...
void Hough3D(pcl::PointCloud<PointT>::Ptr& data, pcl::PointCloud<PointT>::Ptr& obj,
    ...
    pcl::CorrespondencesPtr& model_scene_corrs, std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>>& rototranslations,
    std::vector<pcl::Correspondences>& clustered_corrs, float hough_size)
{
    ...
    rf_est.setRadiusSearch(hough_size);//估计局部参考坐标系时当前点的邻域搜索半径 default = resolution
    ...
    clusterer.setHoughBinSize(0.1f);     //hough空间的采样间隔：0.01
    clusterer.setHoughThreshold(10.0f); //在hough空间确定是否有实例存在的最少票数阈值：5
...
int main()
{
    float down_sampling_leafsize_scene = 0.3f;
    float down_sampling_leafsize_obj = 0.3f;

    float del_plane_distance_scene = 5.0f;
    float del_plane_distance_obj = 5.0f;
    ...
    float keypoint_R_obj = 1.0f; //default = resolution
    float keypoint_R_scene = 1.0f; //default = resolution

    //search radius for description
    float des_R_scene = 10.0f; //default = resolution
    float des_R_obj = 10.0f; //default = resolution

    //matching threshold in KdTree coresspondence
    float matching_threshold = 0.5f;

    //GC custering algorithm
    float GC_size = 5.0f; //consensus set resolution, default = resolution, 越大越松
    int GC_threshold = 100; //minimum cluster size，越小越松 （整数）
    ...
    */
    ...
    viewer.addPointCloudNormals<pcl::PointXYZ, pcl::Normal>(scene, scene_normal,10,2); //interval, length
    ...
    */
    ...
    std::cout << "obj total points: " << obj->size() << "; Selected Keypoints: " << obj_keypoints->size() << std::endl;
    ...
    std::cout << "scene descriptor:" << scene_des->size() << std::endl;
    std::cout << "obj descriptor:" << obj_des->size() << std::endl;
    ...
    //Hough3D(scene, obj, scene_normal, obj_normal, scene_keypoints, obj_keypoints, model_scene_corrs, rototranslations, clustered_corrs, 100.0f);
    ...
    */
    ...
    for (std::size_t i = 0; i < rototranslations.size(); ++i)
    {
        ...
        pcl::visualization::PointCloudColorHandlerCustom<PointT> rotated_model_color_handler(rotated_model, 2, 0, 0); //255,0,0
